...
One of the earliest graph neural network model architectures for constructing machine learning interatomic potentials.
...
class SchNet(nn.Module, IOMixIn):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.node_embedding = nn.Embedding(len(element_types), dim_node_embedding)

        self.graph_layers = nn.ModuleList(
            {
                CFConv(
                    node_in_feats=dim_node_embedding,
                    edge_in_feats=degree_rbf,
                    hidden_feats=units,
                    out_feats=units,
                )
        ...
        self.proj_layers = nn.ModuleList({nn.Linear(units, units) for _ in range(nblocks)})
        if is_intensive:
    ...
    def forward(
        ...
        node_feat = self.node_embedding(node_types)
        fea_dict = {
        ...
        for i in range(self.n_blocks):
            node_feat = self.graph_layers[i](g, node_feat, expanded_dists)
            node_feat = self.proj_layers[i](node_feat)
            fea_dict[f"gc_{i+1}"] = {
                "node_feat": node_feat,
            }
