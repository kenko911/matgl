...
class EdgeSet2Set(nn.Module):
    """Set2Set-style readout for edges (PyG).

    Direct port of :class:`matgl.layers._core_dgl.EdgeSet2Set` using
    ``torch_geometric.utils.softmax`` keyed on the per-edge batch index. Output
    dimension is ``2 * input_dim``.
    """
