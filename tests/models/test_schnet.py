...
import pytest
import torch
from matgl.models import SchNet
...
class TestSchNet:
    def test_model(self, graph_MoS):
        ...
        for act in ["swish", "tanh", "sigmoid", "softplus2", "softexp"]:
            model = SchNet(is_intensive=False, activation_type=act)
    ...
    def test_exceptions(self):
        with pytest.raises(ValueError, match="Invalid activation type"):
            _ = SchNet(element_types=None, is_intensive=False, activation_type="whatever")
        with pytest.raises(ValueError, match="Classification task cannot be extensive."):
            _ = SchNet(element_types=["Mo", "S"], is_intensive=False, task_type="classification")
    ...
    def test_model_intensive(self, graph_MoS):
        ...
        model = SchNet(element_types=["Mo", "S"], is_intensive=True)
        output = model(g=graph)
        assert torch.numel(output) == 1
    ...
    def test_model_intensive_reduced_atom(self, graph_MoS):
        ...
        graph.edata["pbc_offshift"] = torch.matmul(graph.edata["pbc_offset"], lat[0])
        graph.ndata["pos"] = graph.ndata["frac_coords"] @ lat[0]
        model = SchNet(element_types=["Mo", "S"], is_intensive=True, readout_type="reduced_atom")
        output = model(g=graph)
        assert torch.numel(output) == 1
    ...
    def test_model_intensive_set2set_classification(self, graph_MoS):
        ...
        graph.edata["pbc_offshift"] = torch.matmul(graph.edata["pbc_offset"], lat[0])
        graph.ndata["pos"] = graph.ndata["frac_coords"] @ lat[0]
        model = SchNet(element_types=["Mo", "S"], is_intensive=True, task_type="classification", readout_type="set2set")
        output = model(g=graph)
        assert torch.numel(output) == 1

    def test_predict_structure(self, graph_MoS):
        structure, graph, state = graph_MoS
        model_intensive = SchNet(element_types=["Mo", "S"], is_intensive=True)
        model_extensive = SchNet(is_intensive=False)
